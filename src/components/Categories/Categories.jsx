...
const Categories = observer(({ stateStore, api }) => {
  ...
  const _mapCategoryList = () => {
      return stateStore.categoryList.map((category, index) => { 
        ...
        return ( <CategoryThumbnail key={category.id.toString()} id={category.id} index={index} name={category.name} stateStore={stateStore} clickCategory={stateStore.setVisibleCategory({categoryID, categoryName})} /> ); }
      );
  }
